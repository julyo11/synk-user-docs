...
For SCM integrations, Snyk allows you to [run PR Checks](/broken/pages/yPzhschuRU4VKr1IC41w) to validate submitted changes to code and open source packages before merging. Snyk can also retest and alert on the default branch on a scheduled basis. You can see the results on the **Projects** page.
...
* The Snyk CLI can be used in most CI/CD systems (see [examples](https://github.com/snyk-labs/snyk-cicd-integration-examples))
  * Fail the build based on criteria using options or the [snyk-filter](/broken/pages/4I0v4shEdeRktgmi0XIK) tool
...
Where a production integration does not exist, use the [snyk monitor](/broken/pages/A8wYIS7YTucNtXMUFdbe) CLI command to take a snapshot and monitor what is being pushed to production (available for all plans).
...
Snyk also offers fix advice, including recommendations for upgrading vulnerable packages with the [Fix PR feature](/broken/pages/No8dbYg4HfD2SUXo0TaY#snyk-fix-prs).
