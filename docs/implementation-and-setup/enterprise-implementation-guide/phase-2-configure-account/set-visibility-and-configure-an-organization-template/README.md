...
Templating functionality is also available if you are creating your Organizations using the API, whether you are using the [snyk-api-import](/broken/pages/1wlsGRMGFqFhi4vWUyNH) tool to mirror an Organization from an existing source, such as GitHub Organizations, or using the endpoint [Create a new organization](/broken/pages/Cyjt6iHVFWVNEDjGNjKS#org) and providing a `sourceOrgId`.
{% endhint %}
