...
{% hint style="info" %}
See the [Getting started](../../../discover-snyk/getting-started/) section and [Start scanning](/broken/pages/5zCrFjDFiKjZHR4ksDQC) for more details.
{% endhint %}
...
You can connect your repositories for automatic scanning. See [Snyk SCM integrations](/broken/pages/LNOXZ05NnHPVVNO7KcJH) for more details.
...
For hundreds or thousands of repositories, you can use the API endpoint [Import targets](/broken/pages/jLHZgIQBFEsEI7h4RhfR#org-orgid-integrations-integrationid-import) to import your Projects. This takes advantage of an existing source control integration and can be used to automate processes.

The [API-import](/broken/pages/1wlsGRMGFqFhi4vWUyNH) tool uses the API to manage onboarding at scale for large enterprises and is the suggested tool to use at scale. You must mirror the source control structure when using the snyk-api-import tool.

### Snyk CLI

The [Snyk CLI](/broken/pages/eD26UDAHGrXfeHtinya2) allows granular scanning of individual Projects.
...
1. [Install the CLI](/broken/pages/KUVsQmktSlBzw3LlQdio) using one of the appropriate methods as part of the build script.
...
3. Run the appropriate `snyk test` or `snyk monitor` commands with the appropriate options for the type of scan being run.\
   ...
   * Refer to the documentation for [container](/broken/pages/hx8jCYAUHDyUh6BQCRP9) and [infrastructure as code](/broken/pages/wHKKoE0KkZsjCW3HUoUF) scans for information about how to scan those types of Projects.
...
You can scan using the [Snyk API](/broken/pages/WJR3UthqfOfZZs4uhhwu). This enables large-scale automated scanning.
