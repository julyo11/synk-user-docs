...
{% hint style="info" %}
For details, see [Getting started](../../../discover-snyk/getting-started/) and [Start scanning](/broken/pages/5zCrFjDFiKjZHR4ksDQC).
{% endhint %}
...
For details, see [Git repositories (SCMs) integrations with Snyk](/broken/pages/LNOXZ05NnHPVVNO7KcJH).
...
* At scale, Snyk recommends using the API. APIs are available with the Snyk Enterprise plan.
  * Use the [Snyk API](/broken/pages/WJR3UthqfOfZZs4uhhwu) to import your Projects. This leverages an existing source control integration and can be used to automate processes.
  * The [API-import](/broken/pages/1wlsGRMGFqFhi4vWUyNH) tool uses the API to manage onboarding at scale for large enterprises and is the suggested tool to use at scale. The source control structure will need to be mirrored.

## Snyk CLI

For details, see [Snyk CLI](/broken/pages/eD26UDAHGrXfeHtinya2).
...
2. [Authenticate to use the CLI](/broken/pages/XurLzUfRULfzMIxjt6Jy) by using the `snyk auth` command or an environment variable.
...
4. Run the appropriate `snyk test` or `snyk monitor` commands and options for the type of scan you want to run.\
   ...
   * For Container and Infrastructure as Code scans, see [Container](/broken/pages/4AcTDXD3WAov2I57iTvg) and [Infrastructure as Code](/broken/pages/APpxA0ls31leqJCrCZ5f), as this will vary based on the type being scanned.
